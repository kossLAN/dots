...
WrapperItem {
    ...
    Process {
        id: nixSearchProcess
        command: ["nix", "search", "nixpkgs", root.nixQuery, "--json"]
        ...
        stdout: SplitParser {
            splitMarker: ""
            onRead: data => root.nixOutput += data
        }
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            if (root.nixOutput !== "") {
                try {
                    let parsed = JSON.parse(root.nixOutput);
                    let packages = [];
                    ...
                    let query = root.nixQuery.toLowerCase();
                    ...
                    root.nixPackages = packages.slice(0, 50);
                } catch (e) {
    ...
    ColumnLayout {
            ...
            RowLayout {
                ...
                Item {
                    ...
                    TextInput {
                        id: search
                        anchors.fill: parent
                        anchors.leftMargin: root.nixMode ? -prefixWidth : 4
                        ...
                        onAccepted: {
                            ...
                            } else if (root.nixMode && root.nixQuery !== "") {
                                root.runNixPackage(root.nixQuery);
                            }
                        }
                    }
                }
        ...
        StyledListView {
            ...
            model: ScriptModel {
                objectProp: root.nixMode ? "name" : ""

                values: {
                    if (root.nixMode) {
                        return root.nixPackages.map(pkg => ({
                                    name: pkg.attr,
                                    description: pkg.description,
                                    icon: pkg.pname,
                                    version: pkg.version,
            ...
            delegate: MouseArea {
                ...
                function activate(): void {
                    if (isNix) {
                        root.runNixPackage(modelData.nixAttr);
                    } else {
                        modelData.execute();
                        root.accepted();
                    }
